...
//#define LLRP1_TCP_PORT   (5084u) // server mode
//#define LLRP2_TCP_PORT	 (25084u) // client mode
...
/* Changed declaration to just function impl,
 * because the derived class cannot access CPlatformSocket
 * - shheo
 */
#if defined(linux) || defined(__APPLE__)
...
{
	m_portNum = portNum;
	m_portNumStr = new char[6];
	m_pConnectErrorStr = NULL;
	m_isInitiatedConnection = -1;
	sprintf (m_portNumStr, "%d", m_portNum);
    /*
...
CConnection::~CConnection (void)
{
    ...
    delete[] m_portNumStr; // added to accept incoming connection - shheo

    if (m_pPlatformSocket != NULL)
  ...
  const char *                  pReaderHostName)
{
	...
	/* setsockopt of sock to reuse addr */
#if defined(linux) || defined(__APPLE__)
	setsockopt(Sock,SOL_SOCKET,SO_REUSEADDR,(void *)&reuseaddr_flag,sizeof(int));
#endif
#ifdef WIN32
	setsockopt(Sock,SOL_SOCKET,SO_REUSEADDR,(char *)&reuseaddr_flag,sizeof(int));
#endif


	/* fcntl of sock to make it non-blocking */
	/*
#if defined(linux) || defined(__APPLE__)
	int nonblock_flag;
	nonblock_flag = fcntl( Sock, F_GETFL, 0 );
	fcntl( Sock, F_SETFL, nonblock_flag | O_NONBLOCK );
#endif
#ifdef WIN32
	// FIXME: add routine to make socket to non-blocking socket
#endif
*/
...
#endif
#ifdef WIN32
    // FIXME: add routine to make socket timeout
#endif
 ...
 *****************************************************************************
 **
 ** @brief  Accept the connection to the reader
 **
 ** The steps:
 **     - Look up the host name
 **     - Create a socket
...
int CConnection::acceptConnection()
{
	...
	rc = getaddrinfo(NULL, m_portNumStr, &aiHints, &aiList);
	if (rc != 0) {
	...
	ListenSocket = socket(aiList->ai_family, aiList->ai_socktype, aiList->ai_protocol);
	...
	/* setsockopt of listensocket to reuse addr */
#if defined(linux) || defined(__APPLE__)
	setsockopt(ListenSock,SOL_SOCKET,SO_REUSEADDR,(void *)&ListenFlag,sizeof(int));
#endif
#ifdef WIN32
	setsockopt(ListenSock,SOL_SOCKET,SO_REUSEADDR,(char *)&ListenFlag,sizeof(int));
#endif

	/*
	 * Bind the socket to reader. This can stall.
	 */
#if defined(linux) || defined(__APPLE__)
	rc = bind(ListenSock, aiList->ai_addr, (int) aiList->ai_addrlen);
#endif
#ifdef WIN32
	rc = bind(ListenSocket, aiList->ai_addr, (int)aiList->ai_addrlen);
#endif
	...
	{
		/* Connect failed */
		m_pConnectErrorStr = "listen() failed";
#if defined(linux) || defined(__APPLE__)
		close(ListenSock);
#endif
#ifdef WIN32
	...
	{
		/* Connect failed */
		m_pConnectErrorStr = "accept() failed";
#if defined(linux) || defined(__APPLE__)
		close(Sock);
#endif
#ifdef WIN32
...
*/
	 ...
	 * Wrap the platform-specific socket in the platform-specific class.
	 */
	//m_pPlatformSocket_listen = new LLRP::CPlatformSocket(ListenSock);
	m_pPlatformSocket = new LLRP::CPlatformSocket(Sock);

	/*
	 * Victory
	 */
	m_isInitiatedConnection = 0;
	return 0;
}
...
{
	if (m_isInitiatedConnection == 0) {
		if(NULL == m_pPlatformSocket_listen)
		{
...
#if defined(linux) || defined(__APPLE__)
		shutdown(m_pPlatformSocket_listen->m_sock, SHUT_RDWR);
		close(m_pPlatformSocket_listen->m_sock);
		m_pPlatformSocket_listen->m_sock = -1;
#endif
#ifdef WIN32
		closesocket(m_pPlatformSocket_listen->m_sock);
		m_pPlatformSocket_listen->m_sock = NULL;
#endif

		delete m_pPlatformSocket_listen;
		m_pPlatformSocket_listen = NULL;
	}
  ...
  CMessage *                    pMessage)
{
    ...
    if(RC_OK == pError->m_eResultCode)
    {
        ...
        {
        	/* Yikes! */
        	printf ("Socket send() returned error: %d, errno: %d\n", rc, errno);
